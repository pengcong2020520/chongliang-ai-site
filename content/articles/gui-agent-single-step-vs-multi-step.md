...
slug: gui-agent-single-step-vs-multi-step
date: 2026-07-06
updated: 2026-07-06
description: 所有软件最终都会CLI化吗？
source: local
sourceLabel: LOCAL MD
originalUrl:
tags: [GUI Agent, Computer Use, ReAct, 多步可靠性, 训练数据飞轮]
concepts: [dsl]
projects: []
featured: false
hot: false
...
理解了这一点，你才能看懂2024年以来Anthropic、OpenAI、Google为什么疯狂押注GUI Agent。它们不是在做"让AI学会点按钮"的学术研究——它们在争抢一个位置：**用户和所有软件之间的那层入口。**
...
第一条是API/CLI路线——应用主动暴露结构化接口，Agent直接调用。这条路最优，速度快、成本低、可靠性高。Anthropic在2024年底推出MCP（Model Context Protocol），本质上就是在推动这条路——让应用以统一协议暴露能力，把"M个应用×N个AI客户端"的碎片化问题变成M+N。
